  ...
  .then(data => {
    ...
    $('#equipmentTableBody').prepend(`
      <tr data-id="${data.equipoId}">
        ...
        <td>
          <button class="btn btn-warning btn-sm btn-edit-eq">Editar</button>
          <button class="btn btn-danger btn-sm btn-del-eq">Eliminar</button>
        </td>
      </tr>
    `);
...
// Eliminar equipo
$(document).on('click', '.btn-del-eq', function () {
...
// Editar equipo
$(document).on('click', '.btn-edit-eq', function () {
  const row = $(this).closest('tr');
  const id = row.data('id');
